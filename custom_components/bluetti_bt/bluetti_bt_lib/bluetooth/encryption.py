...
def pubkey_to_bytes(pubkey):
    ...
    if out[0] != 0x4 or len(out) != 65:
        raise RuntimeError("First byte should be 04 for the uncompressed format, and total size 64")
    return out[1:]
...
class BluettiEncryption:
    ...
    def msg_challenge(self, message: Message) -> bytes | None:
        ...
        _LOGGER.info("Unsecure key " + (self.unsecure_aes_key.hex() if self.unsecure_aes_key else "None"))
    ...
    def msg_peer_pubkey(self, message: Message) -> bytes | None:
        ...
        signing_secret = int.from_bytes(bytes.fromhex(PRIVATE_KEY_L1), "big")
